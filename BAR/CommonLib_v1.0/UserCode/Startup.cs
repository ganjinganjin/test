...
namespace BAR
{
    public class Startup
    {
        ...
        {
            string str = null;
            g_act.GenLogMessage(GlobConstData.ST_LOG_RECORD, "自动运行开始", "Flow");
            str = "使能吸笔号：";
            for (int i = 0; i < UserConfig.VacuumPenC; i++)
            {
                if (Run.PenEnable[i].Checked)
            ...
            g_act.GenLogMessage(GlobConstData.ST_LOG_RECORD, str, "Flow");
            g_act.GenLogMessage(GlobConstData.ST_LOG_RECORD, Auto_Flag.TestMode ? "测试模式" : "烧录模式", "Flow");
            g_act.GenLogMessage(GlobConstData.ST_LOG_RECORD, Auto_Flag.BurnMode ? "烧录模式" : "转移模式", "Flow");
            if (UserTask.ShiftWay == 5)
            {
                str = "管转盘," + (Auto_Flag.AutoTray ? "自动送盘" : "固定盘");
            }
            ...
            {
                str = "编转盘," + (Auto_Flag.AutoTray ? "自动送盘" : "固定盘");
            }
            ...
            {
                str = "盘转编," + (Auto_Flag.AutoTray ? "自动送盘" : "固定盘");
            }
            else
            {
                str = "盘转盘," + (Auto_Flag.AutoTray ? "自动送盘" : "固定盘");
            }
            g_act.GenLogMessage(GlobConstData.ST_LOG_RECORD, str, "Flow");
            g_act.GenLogMessage(GlobConstData.ST_LOG_RECORD, Auto_Flag.NGTray ? "NG盘放NG料" : "NG杯放NG料", "Flow");

            str = "取料旋转角度：";
            if (Auto_Flag.AutoTray_TakeIC || Auto_Flag.FixedTray_TakeIC)
            {
                str += "料盘1 = " + RotateAngle.TIC_Tray[0] + "°    " + "料盘2 = " + RotateAngle.TIC_Tray[1] + "°";
            }
            else if (Auto_Flag.Brede_TakeIC)
            {
                str += "编带 = " + RotateAngle.TIC_Brede + "°";
            }
            else if (Auto_Flag.FixedTube_TakeIC)
            {
                str += "料管 = " + RotateAngle.TIC_Tube + "°";
            }
            g_act.GenLogMessage(GlobConstData.ST_LOG_RECORD, str, "Flow");
            ...
            {
                str += "料盘1 = " + RotateAngle.LIC_Tray[0] + "°    " + "料盘2 = " + RotateAngle.LIC_Tray[1] + "°    ";
            }
            else if (Auto_Flag.Brede_LayIC)
            {
                str += "编带 = " + RotateAngle.LIC_Brede + "°    ";
            }
            else if (Auto_Flag.FixedTube_LayIC)
            {
                str += "料管 = " + RotateAngle.LIC_Tube + "°    ";
            }
            str += "NG盘 = " + RotateAngle.LIC_Tray[2] + "°" ;
            g_act.GenLogMessage(GlobConstData.ST_LOG_RECORD, str, "Flow");

            if (Auto_Flag.Brede_LayIC)
            {
                g_act.GenLogMessage(GlobConstData.ST_LOG_RECORD, Auto_Flag.Brede_Check ? "编带走带检测开" : "编带走带检测关", "Flow");
                g_act.GenLogMessage(GlobConstData.ST_LOG_RECORD, Auto_Flag.Velum_Check ? "编带盖膜检测开" : "编带盖膜检测关", "Flow");
                g_act.GenLogMessage(GlobConstData.ST_LOG_RECORD, Auto_Flag.DotA ? "编带打点开" : "编带打点关", "Flow");
            }

            if (Auto_Flag.Production)
            {
                str = "定量生产," + (Auto_Flag.ProductionOK ? "OK数" : "取料数") + "，目标数：" + UserTask.TargetC;
            }
            else
            {
                str = "非定量生产";
            ...
            g_act.GenLogMessage(GlobConstData.ST_LOG_RECORD, str, "Flow");
            str = "OK完成数：" + UserTask.OKDoneC + "，取料完成数：" + UserTask.TIC_DoneC;
            g_act.GenLogMessage(GlobConstData.ST_LOG_RECORD, str, "Flow");
